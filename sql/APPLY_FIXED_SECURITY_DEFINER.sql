...
COMMENT ON FUNCTION get_temporal_analysis_asof(DATE, DATE, TEXT, TEXT[], TEXT[], TEXT[], BOOLEAN, BOOLEAN, BOOLEAN) IS 'Versão v3 com SECURITY DEFINER para ignorar RLS e permitir acesso via RPC autenticado';
...
COMMENT ON FUNCTION get_sankey_snapshot(DATE, UUID[], TEXT, TEXT[], TEXT[], TEXT[], BOOLEAN, BOOLEAN, BOOLEAN) IS 'Snapshot de Health Score para Movement Sankey com SECURITY DEFINER para ignorar RLS';
...
COMMENT ON FUNCTION get_client_health_score_evolution(UUID) IS 'Função com SECURITY DEFINER para ignorar RLS';
...
COMMENT ON FUNCTION diff_snapshot_pairs(TEXT[]) IS 'Função com SECURITY DEFINER para ignorar RLS';
...
DROP POLICY IF EXISTS "Authenticated users can view available names" ON user_profiles;
...
COMMENT ON FUNCTION get_hierarchy_cascade(TEXT, TEXT) IS 'Função com SECURITY DEFINER para ignorar RLS';
...
COMMENT ON FUNCTION bulk_insert_clients_v3(JSONB, DATE, TIMESTAMPTZ) IS 'Função de bulk insert com SECURITY DEFINER para ignorar RLS';
