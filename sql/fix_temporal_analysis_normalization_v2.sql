-- ============================================
-- CORREÇÃO v2: Normalização de filtros hierárquicos (versão mais defensiva)
-- ============================================
-- Esta versão corrige um possível problema com arrays vazios vs NULL
-- e torna a lógica de filtros mais explícita
--
-- DATA: 2025-11-26
-- AUTOR: Claude AI

-- Garantir que extensão unaccent existe
CREATE EXTENSION IF NOT EXISTS unaccent;

-- Recriar função normalize_text (caso não exista)
...
-- Recriar função get_temporal_analysis_asof com lógica corrigida
...
BEGIN
  ...
  -- e marcar se temos filtros ativos (diferente de NULL ou vazio)
  ...
  last_snapshots AS (
    ...
    WHERE (
      ...
      AND s.planner <> '0'
      -- Filtro de manager: só aplicar se has_manager_filter for TRUE
      AND (
      ...
      -- Filtro de mediator: só aplicar se has_mediator_filter for TRUE
      AND (
      ...
      -- Filtro de leader: só aplicar se has_leader_filter for TRUE
      AND (
...
COMMENT ON FUNCTION get_temporal_analysis_asof IS 'Versão corrigida com normalização de texto e lógica defensiva para arrays vazios/NULL';
