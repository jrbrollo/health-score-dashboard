-- ============================================
-- CORREÇÃO: Priorizar último registro quando há múltiplos registros no mesmo dia
-- ============================================
-- PROBLEMA:
-- A função get_temporal_analysis_asof não estava garantindo que, quando há múltiplos
-- registros com o mesmo recorded_date para o mesmo cliente, seja usado o mais recente
-- (baseado em created_at ou id).
--
-- CAUSA:
-- ORDER BY h.client_id, h.recorded_date DESC
-- Não inclui created_at ou id, então se houver múltiplos registros com o mesmo
-- recorded_date, não há garantia de qual será selecionado.
--
-- SOLUÇÃO:
-- Adicionar created_at DESC (ou id DESC) ao ORDER BY para garantir que seja usado
-- o registro mais recente quando há múltiplos registros com o mesmo recorded_date.
...
BEGIN
  ...
  last_snapshots AS (
    ...
    JOIN LATERAL (
      ...
      -- CORREÇÃO CRÍTICA: Adicionar created_at DESC para garantir que seja usado
      -- o registro mais recente quando há múltiplos registros com o mesmo recorded_date
