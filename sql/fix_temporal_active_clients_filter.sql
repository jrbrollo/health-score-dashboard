-- ============================================
-- CORREÇÃO: Filtrar apenas clientes ATIVOS em get_temporal_analysis_asof
-- ============================================
-- PROBLEMA: A função estava retornando ~1860 clientes (incluindo inativos)
-- quando deveria retornar apenas ~1009 clientes ativos (do último snapshot).
--
-- SOLUÇÃO: Adicionar filtro para considerar apenas clientes que aparecem
-- no último snapshot (última data de last_seen_at).
--
-- DATA: 2025-11-26
-- ============================================
...
DECLARE
  ...
  max_last_seen_date DATE;
BEGIN
  -- Obter a data máxima de last_seen_at (data do último snapshot)
  -- Isso garante que apenas clientes ativos sejam considerados
  SELECT MAX((last_seen_at AT TIME ZONE 'UTC')::DATE)
  INTO max_last_seen_date
  FROM clients
  ...
  RETURN QUERY
  WITH
  -- Identificar apenas clientes ativos (que aparecem no último snapshot)
  active_clients AS (
    SELECT DISTINCT client_id
    FROM clients
    WHERE (last_seen_at AT TIME ZONE 'UTC')::DATE = max_last_seen_date
  ...
  last_snapshots_by_id AS (
    ...
    JOIN LATERAL (
      ...
      -- FILTRO CRÍTICO: Apenas clientes ativos
...
COMMENT ON FUNCTION get_temporal_analysis_asof IS 'Versão final com SECURITY DEFINER e filtro de clientes ativos baseado em last_seen_at';
