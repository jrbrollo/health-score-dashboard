-- Atualizar função para remover prefixo "TEL:" do telefone
...
BEGIN
  ...
  SELECT
    (client->>'name'),
    (client->>'planner'),
    -- Remover prefixo "TEL:" se existir
    CASE 
      WHEN (client->>'phone') LIKE 'TEL:%' THEN SUBSTRING((client->>'phone'), 5)
      ELSE (client->>'phone')
    END,
    (client->>'email'),
    (client->>'leader'),
...
-- Verificar se o telefone foi salvo sem o prefixo
SELECT name, phone FROM clients WHERE name = 'TESTE Prefix';
