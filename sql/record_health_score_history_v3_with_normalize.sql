...
-- COM NORMALIZAÇÃO ROBUSTA DE NOMES
-- ========================================
-- ATUALIZAÇÃO: Usa normalize_text() para buscar pagante de cônjuge
-- IMPORTANTE: Execute normalize_text_function_v2.sql ANTES desta função
--
...
BEGIN
  ...
  IF v_client.is_spouse = TRUE
    ...
    -- ✨ ATUALIZAÇÃO CRÍTICA: Buscar NPS do pagante com normalização robusta
    -- ANTES: WHERE lower(trim(name)) = lower(trim(v_client.spouse_partner_name))
    -- AGORA:  WHERE normalize_text(name) = normalize_text(v_client.spouse_partner_name)
    -- BENEFÍCIO: Remove acentos, múltiplos espaços, case insensitive
    SELECT nps_score_v3 INTO v_payer_nps
    FROM clients
    WHERE normalize_text(name) = normalize_text(v_client.spouse_partner_name)
    ...
    IF v_payer_nps IS NOT NULL THEN
      ...
      RAISE NOTICE 'Cônjuge % herdou NPS % do pagante %', v_client.name, v_payer_nps, v_client.spouse_partner_name;
    ELSE
      RAISE WARNING 'Cônjuge % não encontrou pagante "%" (planner: %)', v_client.name, v_client.spouse_partner_name, v_client.planner;
    END IF;
  END IF;
  ...
  IF v_client.months_since_closing IS NOT NULL AND v_client.months_since_closing >= 0 THEN
    ...
    ELSIF v_client.months_since_closing >= 13 THEN
      v_tenure_pillar := 15; -- Maduro (13-24)
    ELSIF v_client.months_since_closing >= 9 THEN
      v_tenure_pillar := 15; -- Consolidado (9-12)
    ELSIF v_client.months_since_closing >= 5 THEN
      v_tenure_pillar := 10; -- Consolidação inicial (5-8)
    ELSIF v_client.months_since_closing >= 0 THEN
      v_tenure_pillar := 5; -- Onboarding (0-4)
    END IF;
  END IF;
  ...
  IF p_recorded_date < CURRENT_DATE THEN
    ...
    INSERT INTO health_score_history (
      client_id, recorded_date, client_name, planner, health_score, health_category,
      meeting_engagement, app_usage, payment_status, ecosystem_engagement, nps_score,
      last_meeting, has_scheduled_meeting, app_usage_status, payment_status_detail,
      has_referrals, nps_score_detail, ecosystem_usage, email, phone, is_spouse,
      leader, mediator, manager, months_since_closing, nps_score_v3, has_nps_referral,
      overdue_installments, overdue_days, cross_sell_count, nps_score_v3_pillar,
      referral_pillar, payment_pillar, cross_sell_pillar, tenure_pillar
    )
    VALUES (
      v_client.id, p_recorded_date, v_client.name, v_client.planner, v_health_score, v_health_category,
      0, 0, 0, 0, 0, 'Nunca', FALSE, 'Nunca usou', 'Em dia', FALSE, 'Não avaliado', 'Não usa',
      v_client.email, v_client.phone, v_client.is_spouse, v_client.leader, v_client.mediator,
      v_client.manager, v_client.months_since_closing, v_nps_value, v_client.has_nps_referral,
      v_client.overdue_installments, v_client.overdue_days, v_client.cross_sell_count,
      v_nps_pillar, v_referral_pillar, v_payment_pillar, v_cross_sell_pillar, v_tenure_pillar
    )
...
COMMENT ON FUNCTION record_health_score_history_v3 IS 'Registra health score v3 no histórico. Usa normalize_text() para herança robusta de NPS de cônjuges.';
